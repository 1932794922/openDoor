...
uint16_t findHzIndex(uint8_t *hz, uint8_t size)       /* 在自定义汉字库在查找所要显示 */                                                     /* 的汉字的位置 */
{
    uint32_t i=0;
    ...
    while(ptGb32[i].Index[0] > 0x80) 
		{
        if ((*hz == ptGb32[i].Index[0]) && (*(hz+1) == ptGb32[i].Index[1])) 
				{
            return i;
        }
        i++;
...
void WriteOneHzChar(uint16_t x,uint16_t y,uint8_t *font ,  uint16_t RGB_Code,uint16_t back_color)
{
	...
	ch = GBHZ_16[findHzIndex(font,16)].Msk ;
...
//Personal creation: By H 

//实心矩形左上(x1,y1)、右下(x2,y2)
void LCD_Draw_fillRectangle1(uint16_t x1,uint16_t y1,uint16_t x2,uint16_t y2,uint16_t RGB_Code)
{
	int i,j;
	for(i=x1 ;i<=x2;i++)
		for(j=y1 ;j<=y2;j++)
				LCD_DrawPoint(i,j,RGB_Code);
}

//(x,y)实心矩形中心坐标，wide ：边长
void LCD_Draw_fillRectangle2(uint16_t x,uint16_t y,uint8_t wide ,uint16_t RGB_Code)
{
	int i;
	for(i=0;i<=wide;i++)
	LCD_DrawLine(x-wide/2,y-wide/2+i,x+wide/2,y-wide/2+i,RGB_Code);
}
...
void draw_circle(int x, int y, int r, int color)
{
    ...
    {
        LCD_DrawPoint(x + a, y - b,color); // 0~1
        LCD_DrawPoint(x - a, y - b,color); // 0~7
        LCD_DrawPoint(x - a, y + b,color); // 4~5
        LCD_DrawPoint(x + a, y + b,color); // 4~3
 
        LCD_DrawPoint(x + b, y + a,color); // 2~3
        LCD_DrawPoint(x + b, y - a,color); // 2~1
        LCD_DrawPoint(x - b, y - a,color); // 6~7
        LCD_DrawPoint(x - b, y + a,color); // 6~5
...
//画一个屏幕按键
void Draw_button(uint16_t x1,uint16_t y1,uint16_t x2,uint16_t y2,char* ch,uint16_t len,uint16_t RGB_Code,uint16_t back_color)
{
	LCD_DrawRectangle(x1,y1,x2,y2,RGB_Code);
	LCD_ShowString((x1+x2)/2-len*4+4,(y1+y2)/2 - 8,(uint8_t*)ch,RGB_Code,back_color);
}

//显示32*32字体
void WriteHz32Char(uint16_t x,uint16_t y,uint8_t *font, uint16_t RGB_Code, uint16_t back_color)
{
	uint8_t temp,i,j;
	uint16_t x0=x;  
	uint8_t *ch;     
	
	ch = GBHZ_32[findHzIndex(font,32)].Msk ;

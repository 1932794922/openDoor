...
//   UART_send_byte(status);
    //usart_printf(&DEBUG_UART,"status=%c\r\n",status);
...
char PcdAuthState(unsigned char auth_mode, unsigned char addr, unsigned char *pKey, unsigned char *pSnr) {
    ...
    //usart_printf(&DEBUG_UART,"ucComMF522Buf=%s\r\n",ucComMF522Buf);
...
unsigned char ReadRawRC(unsigned char Address) {
    ...
    HAL_SPI_Transmit(&hspi1, &ucAddr, 1, 0xff);//向总线写多个数据        //hspi2
    //SPI_WriteNBytes(SPI1,&ucAddr,1);
    HAL_SPI_Receive(&hspi1, &ucResult, 1, 0xff);//向总线读多个数据        //hspi2
...
void WriteRawRC(unsigned char Address, unsigned char value) {
    ...
    HAL_SPI_Transmit(&hspi1, write_buffer, 2, 0xff);//向总线写多个数据        //hspi2
    RC522_DISABLE;
}
...
char PcdComMF522(unsigned char Command,
    ...
    for (i = 0; i < InLenByte; i++) { WriteRawRC(FIFODataReg, pInData[i]); }
